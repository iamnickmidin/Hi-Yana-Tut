...
## Описание задачи (по-человечески)

Программа сначала делает квадратную таблицу **K×K** и заполняет её случайными цифрами от **0 до 9**.

Потом нужно найти путь из **левого верхнего угла** (1,1) в **правый нижний** (K,K), причём ходить можно **только вправо или вниз**.

У каждого пути считается **сумма чисел**, которые мы прошли по клеткам.
Нам надо:

1. **вывести саму матрицу**
2. **вывести все возможные пути** от начала до конца
3. при этом вывести пути **в порядке увеличения суммы** (то есть сначала самый “дешёвый”, потом следующий и т.д.)

Путь можно показывать, например, как строку из букв:

* `R` — шаг вправо
* `D` — шаг вниз

Например `RRDD` значит: вправо, вправо, вниз, вниз.

---

### Важный момент

Путей на самом деле очень много, и их количество быстро растёт с увеличением K.
Поэтому обычно берут **небольшие K**, иначе вывести все пути станет слишком долго.
